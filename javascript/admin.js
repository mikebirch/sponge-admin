...
$(document).ready(function() {
	...
	// create text links for all delete forms
	$("td.actions form").each(function() {
	    $(this).after('<a href="#" class="delete" data-formname="'+$(this).attr('id')+'">Delete</a>');
	});
  ...
  // delete options
	$('.delete').click(function(){
 		...
 		$(this).parent().parent().addClass('delete_selected').after('<tr id="delete_choices"><td colspan="' + 
 				cells + '">Do you really want to delete this? ' +
 				'<a href="#" class="btn-cancel button" id="cancel_delete">No</a> ' +
      		'<a href="#" class="btn button" id="confirm_delete" data-form="' + formname + '">Yes</a>' +
      		'</td></tr>');
	...
	// fade out success flash messages
	var success = $('.message');
	if(success.length) {
  ...
  // responsive main navigation menu, see responsive-nav.js
  ...
  if(window.getComputedStyle) {
    // generated content is added to the body within a media query - see http://adactio.com/journal/5429/ 
    function isNarrowScreen() {
      var size = window.getComputedStyle(document.body,':after').getPropertyValue('content');
      if (size.indexOf("smallscreen") !=-1 || size.indexOf("mediumscreen") !=-1 || size.indexOf("widescreen") !=-1 ) {
        return false;
      } else {
        return true;
      }
  ...
  // close the user menu if click anywhere else on page
